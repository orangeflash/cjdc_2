...
Page({
  ...
  reLoad: function () {
    var that = this, status = this.data.status || 1, state = this.data.state || '', searchinfo = this.data.searchinfo || '', store_id = wx.getStorageSync('sjdsjid'), page = this.data.pagenum, start_time = this.data.start_time, end_time = this.data.end_time;
    // var dn_states;
    // if (status == 1) {
    //   dn_states = '1,2,3'
    // }
    ...
    //   dn_states = '3';
    // }
    console.log(status, state, store_id, page, searchinfo, start_time, end_time)
    app.util.request({
      'url': 'entry/wxapp/storeYyOrder',
      'cachetime': '0',
      data: { yy_state: state, keywords: searchinfo, start: start_time, end: end_time, store_id: store_id, page: page, pagesize: 10 },
  ...
  jjjd: function (e) {
    var a = this, oid = e.currentTarget.dataset.id, yy_state = e.currentTarget.dataset.type;
    console.log(oid,yy_state)
    wx.showModal({
      title: "提示",
      content: yy_state == '4' ? "是否拒绝订单？" : "是否确认订单？",
      cancelText: "否",
      confirmText: "是",
      success: function (s) {
        ...
        s.confirm && (wx.showLoading({
          ...
          'cachetime': '0',
          data: { order_id: oid, yy_state: yy_state },
          success: function (res) {
            console.log(res.data)
            if (res.data == '1') {
              wx.showToast({
